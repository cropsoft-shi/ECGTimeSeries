...
if __name__ == '__main__':
    ...
    # df = pd.read_csv('data/ecg5000_olsr.csv')
    # CLASS_NORMAL = 1
    # 正常/异常划分
    CLASS_NORMAL = '1'
    ...
    normal_df, anomaly_df= normal_df_raw,anomaly_df_raw
    ...
    model = torch.load('log/TimeSeriesTrain_0610_221900.pth',
                       map_location=device,
                       weights_only=False  # 启用安全模式
    ...
    THRESHOLD=26#26  160
    correct1 = sum(l <= THRESHOLD for l in losses)
    ...
    percent2 = correct2 / len(test_anomaly_dataset)
    print(f'Correct anomaly predictions: {correct2}/{len(test_anomaly_dataset)}')
    ...
    sns.histplot(losses, kde=True, bins=100, color='blue', label='Normal Test Loss', alpha=0.6)

    # Plot Validation Loss with KDE
    sns.histplot(pred_losses, kde=True, bins=100, color='red', label='Anomaly Test Loss', alpha=0.6)
    ...
    # plt.xlim(0, 200)
    plt.xlim(0, 200)# 800 200
    # Add labels and title
    # plt.title("Autoencoder: Test loss distribution with KDE",fontsize=20)
    ...
    plt.savefig('kde1.tiff', bbox_inches='tight', format='tiff', dpi=300)
    plt.show()
    exit()
    ...
    # print(losses.sort(reverse=True)[:10])
