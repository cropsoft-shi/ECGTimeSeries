...
csv_filename = os.path.join(log_dir, f'{current_file_name}_{current_time}.csv')
print(f'{csv_filename}')
with open(csv_filename, 'w', newline='') as f:
    writer = csv.writer(f)
    # writer.writerow(["Epoch", "Train Loss", "Val Loss", "Normal Acc", "Anomaly Acc", "F1", "AUC","Threshold"])
    writer.writerow(["Epoch", "Train Loss", "Val Loss",
                     "F1",'AUC',
                     "Normal Acc", "Anomaly Acc",
                     "normal_acc_train", "normal_acc_val",
                     "threshold"])
# 绘图风格
...
def determine_threshold_best(model, val_ds, test_norm,test_anom, device):
    norm_losses = predict(model, test_norm, device)
    anom_losses = predict(model, test_anom[:len(test_norm)], device)

    y_true = [0] * len(norm_losses) + [1] * len(anom_losses)  # 0=正常，1=异常
    # 合并正常和异常样本的重构误差
    all_losses = np.array(norm_losses + anom_losses)

    # 设置阈值的候选范围，这里用的是从最小值到最大值的均匀分布
    thresholds = np.linspace(all_losses.min(), all_losses.max(), 100)

    best_f1, best_thr = 0, thresholds[0]
    ...
    for thr in thresholds:
        ...
        # 计算当前阈值下的F1分数
        _, _, f1, _ = precision_recall_fscore_support(y_true, preds,pos_label=0, average='binary')
...
def loss_function(recon, x, mu, logvar, beta=kl_weight):
    ...
    """
    方案三：先对每个样本累加所有时序点和通道的 MSE，再对 batch 求平均。
    这样既不会被单点 outlier 拉得过大，也能保持和 sum-reduction 同量级的损失。

    recon:     (B, T, C)  重构结果
    x:         (B, T, C)  原始输入
    mu, logvar:(B, D)     潜变量分布参数
    """


    huber_sum = nn.SmoothL1Loss(reduction='sum')
    recon_loss = huber_sum(recon, x) / recon.size(0)
...
def train_model(model, train_ds, val_ds, test_norm,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch % 10 == 0 or threshold is None:
            ...
            # threshold = determine_threshold_best(model, val_ds,test_norm,test_anom, device)
        ...
        with open(csv_filename, 'a', newline='') as f:
            ...
            writer.writerow([epoch,
                             ...
                             results['normal_acc'], results['anomaly_acc'],
                             results_train['normal_acc_train'],results_train['normal_acc_val'],
                             threshold])
...
if __name__ == '__main__':
    ...
    model, history, best_threshold = train_model(
        model, train_ds, val_ds, test_norm_ds, test_anom_ds, device, n_epochs=400)

    # checkpoint = torch.load(r'log\TimeSeriesTrainAtt_0619_103552_0.9735.pth', map_location=device)
    ...
    # 保存模型
    # model_path = os.path.join(log_dir, f'{current_file_name}_{current_time}.pth')
    # torch.save({
    ...
    if DRAW:
        ...
        plt.subplot(2, 2, 2)
        plt.plot(history['normal_acc'], label='正常样本准确率')
        plt.plot(history['anomaly_acc'], label='异常样本准确率')
        plt.xlabel('Epoch')
        plt.ylabel('准确率')
        plt.legend()
        plt.title('分类准确率')
        ...
        plt.subplot(2, 2, 3)
        plt.plot(history['f1'], label='F1分数')
        plt.plot(history['auc'], label='AUC分数')
        plt.xlabel('Epoch')
        plt.ylabel('分数')
        plt.legend()
        plt.title('F1与AUC评估指标')
        ...
        with torch.no_grad():
            # 从测试集中选择一些正常和异常样本
            norm_sample = test_norm_ds[0].unsqueeze(0).to(device)
            anom_sample = test_anom_ds[0].unsqueeze(0).to(device)

            # 重建
            norm_recon, _, _ = model(norm_sample)
            anom_recon, _, _ = model(anom_sample)

            # 转到CPU并移除批次维度
            norm_sample = norm_sample.cpu().squeeze(0).squeeze(1).numpy()
            norm_recon = norm_recon.cpu().squeeze(0).squeeze(1).numpy()
            anom_sample = anom_sample.cpu().squeeze(0).squeeze(1).numpy()
            anom_recon = anom_recon.cpu().squeeze(0).squeeze(1).numpy()

            # 绘制
            plt.plot(norm_sample, 'b-', alpha=0.5, label='正常样本')
            plt.plot(norm_recon, 'b--', label='正常重建')
            plt.plot(anom_sample, 'r-', alpha=0.5, label='异常样本')
            plt.plot(anom_recon, 'r--', label='异常重建')
            plt.legend()
            plt.title('样本重建结果')

        plt.tight_layout()
        plt.savefig(os.path.join(log_dir, f'{current_file_name}_{current_time}_plot.png'))
        plt.show()
